...
<div class="activity-log-container">
    <!-- Breadcrumb -->
    <nav class="activity-log-breadcrumb">
    ...
    <div class="activity-log-header">
        ...
        <p class="activity-log-subtitle">{{ $logs->total() }} {{ trans_choice('activity_log.records', $logs->total()) }}</p>
    </div>

    <!-- Filters -->
    <form method="GET" class="activity-log-filters">
                            ...
                            <tr>
                                <td class="action-cell">
                                    <div class="action-badge">
                                        @php
                                            $actionType = Str::before($log->action, ':');
                                            $actionDetail = Str::after($log->action, ':');
                                        @endphp
                                        <span class="action-type">{{ $actionType }}</span>
                                        @if($actionDetail !== $log->action)
                                ...
                                <td class="date-cell">
                                    <time datetime="{{ $log->created_at->toIso8601String() }}">
                                        ...
                                        <span class="time-muted">{{ $log->created_at->format('H:i') }}</span>
                                    </time>
                                </td>
                            </tr>
                        @endforeach
